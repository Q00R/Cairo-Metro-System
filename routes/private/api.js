  ...
  app.delete("/api/v1/station/:stationId", async function (req, res) {
    try {
      ...
      if (user.isAdmin) {
        ...
        console.log(ttickets);
        for (let i = 0; i < ttickets.length; i++) {
          let cur = ttickets[i];
          const tticketId = cur.id;
          try {
            console.log(cur);
            let org = await db("se_project.stations").select("id").where("stationName", cur.origin).first();
            let dest = await db("se_project.stations").select("id").where("stationName", cur.destination).first();
            
            let retrievingRoute = await calculatePrice(org.id, dest.id);
            ...
            await db("se_project.transactions").update("amount", priceThatShouldBePayed).where("purchasedId", tticketId).andWhere("type", "ticket");
          }
          catch (e) {
            console.log(e.message);
  ...
  app.post('/api/v1/tickets/purchase/subscription', async function (req, res) {
    try {
      ...
      if (hasSubscription && Object.keys(hasSubscription).length !== 0) {
        if (hasSubscription.noOfTickets == 0) {
          await db('se_project.tickets').where('subId', hasSubscription.id).update({ 'subId': null });
          await db('se_project.subsription')
            .where('userId', currentID)
            .del();
          return res.status(400).send('No tickets remaining you have to pay using your credit card kindly refresh the page to pay using your credit card');
        }
        console.log(originResult, destinationResult);
        if (origin === destination) {
          return res.status(400).send("price: 0, " + `You are already at your destination ${destination}`);
        } else {
          console.log("hasSubscription.noOfTickets", hasSubscription.noOfTickets);
